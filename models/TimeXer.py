...
TimeXer is a time series forecasting model that combines patch-based embeddings
with cross-variable attention mechanisms. It uses an encoder-only architecture
with both self-attention on patches and cross-attention on variables.

Key Features:
    - Patch-based encoder: Divides time series into patches
    - Cross-variable attention: Captures dependencies between variables
    - Global token mechanism: Aggregates patch information
    - Non-stationary normalization: Handles non-stationary time series

Paper Reference:
    TimeXer: Design Space Exploration of Time Series Exogenous Modeling
